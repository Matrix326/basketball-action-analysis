...
class TemporalClipVideo(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.text_aug = cfg.DATA.TEXT_AUG
        self.text_feature_file = cfg.DATA.TEXT_AUG_FEATURE_FILE
        ...
        if self.tune_head:
            if self.text_aug:
                self.dynamic_classifier = self.achieve_xcsf_matrix(self.text_feature_file)
            else:
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.MODEL.ARCH == 'vitb32':
            self.model, self.preprocess = load("ViT-B/32", jit=False, 
                    ...
                    mask_rate=cfg.MODEL.MASK_RATE, mask_stride=cfg.MODEL.MASK_STRIDE,
                    spatial_repeat=cfg.MODEL.SPATIAL_REPEAT, temporal_shuffle=cfg.MODEL.TEMPORAL_SHUFFLE,
                    channel_fold=cfg.MODEL.CHANNEL_FOLD, temporal_scale=cfg.MODEL.TEMPORAL_SCALE,
            ...
            if cfg.MODEL.KEEP_RAW_MODEL:   
                self.raw_model, self.preprocess = load("ViT-B/32", jit=False, 
                        ...
                        mask_rate=cfg.MODEL.MASK_RATE, mask_stride=cfg.MODEL.MASK_STRIDE,
                        spatial_repeat=cfg.MODEL.SPATIAL_REPEAT,
                        temporal_shuffle=cfg.MODEL.TEMPORAL_SHUFFLE,
                        channel_fold=cfg.MODEL.CHANNEL_FOLD, temporal_scale=cfg.MODEL.TEMPORAL_SCALE,
                        use_checkpoint=cfg.MODEL.USE_CHECKPOINT, context_length=context_length,
                        num_experts=cfg.MODEL.NUM_EXPERTS, expert_insert_layers=cfg.MODEL.EXPERT_INSERT_LAYERS,
                        record_routing=cfg.MODEL.RECORD_ROUTING, routing_type=cfg.MODEL.ROUTING_TYPE
            ...
            self.model, self.preprocess = load("ViT-B/16", jit=False, 
                    ...
                    mask_rate=cfg.MODEL.MASK_RATE, mask_stride=cfg.MODEL.MASK_STRIDE,
                    spatial_repeat=cfg.MODEL.SPATIAL_REPEAT,
                    temporal_shuffle=cfg.MODEL.TEMPORAL_SHUFFLE, channel_fold=cfg.MODEL.CHANNEL_FOLD, temporal_scale=cfg.MODEL.TEMPORAL_SCALE,
                    use_checkpoint=cfg.MODEL.USE_CHECKPOINT, context_length=context_length,
                    num_experts=cfg.MODEL.NUM_EXPERTS, expert_insert_layers=cfg.MODEL.EXPERT_INSERT_LAYERS,
                    record_routing=cfg.MODEL.RECORD_ROUTING, routing_type=cfg.MODEL.ROUTING_TYPE
                    )
    ...
    def forward(self, x=None, update=False):
        ...
        if self.training:
            ...
            if self.tune_head:
                ...
                if len(self.dynamic_classifier.shape) < 3:
                    pred = self.model.logit_scale.exp() * img_encode @ self.dynamic_classifier.T
                else:
                    img_encode = img_encode.reshape(bz, clip_len, -1)
                    sims = torch.einsum('btd,knd->btkn', self.model.logit_scale.exp()*img_encode, self.dynamic_classifier)

                    if self.cfg.TRAIN.CROSS_BATCH_META_DEBIAS:
                        pred = torch.mean(sims, dim=-1, keepdim=False)  # b t k
                    else:
                        max_n, _ = torch.max(sims, dim=-1, keepdim=False)  # b t k
                        max_t, _ = torch.max(sims, dim=1, keepdim=False)  # b k n
                        max_mean_n = torch.mean(max_t, dim=-1, keepdim=False)  # b k
                        max_mean_t = torch.mean(max_n, dim=1, keepdim=False)  # b k
                        pred = (max_mean_n + max_mean_t) / 2.  # b k


            if self.cfg.TRAIN.CROSS_BATCH_META_DEBIAS:
                pred = pred.reshape(bz, clip_len, -1)
            else:
                pred = pred.reshape(bz, clip_len, -1).mean(1) if pred.shape[0] != bz else pred
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation):
                with torch.no_grad():
                    ...
                    if len(self.dynamic_classifier.shape) < 3:
                        ...
                        raw_img_encode = raw_img_encode.reshape(bz, clip_len, -1)
                        sims = torch.einsum('btd,knd->btkn', self.model.logit_scale.exp() * raw_img_encode,
                                            self.dynamic_classifier)
                        max_t, _ = torch.max(sims, dim=1, keepdim=False)  # b k n
                        max_mean_n = torch.mean(max_t, dim=-1, keepdim=False)  # b k
                        max_n, _ = torch.max(sims, dim=-1, keepdim=False)  # b t k
                        max_mean_t = torch.mean(max_n, dim=1, keepdim=False)  # b k
                        raw_pred = (max_mean_n + max_mean_t) / 2.  # b k
            ...
            if self.tune_head:
                ...
                norm_head = self.model.prompt_encode_text(text_embedding, self.tokenized_prompts,)
                norm_head = norm_head / norm_head.norm(dim=-1, keepdim=True)
                pred = self.model.logit_scale.exp() * img_encode @ norm_head.T
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation):
                with torch.no_grad():
                    ...
                    raw_pred = raw_pred.reshape(bz, clip_len, -1).mean(1) if raw_pred.shape[0] != bz else raw_pred
    ...
    #
    #     return csf_matrix
    def achieve_xcsf_matrix(self, text_feature_file):
        if os.path.isfile(text_feature_file):
            text_feature = torch.load(text_feature_file)
            text_feature = text_feature.cuda()
            xcsf_matrix = text_feature / text_feature.norm(dim=-1, keepdim=True) # k a d
            return xcsf_matrix
        else:
            return self.achieve_csf_matrix(self.text_dict, self.model)
...
def build_model(state_dict: dict, T=8, temporal_modeling_type=None,
                mask_rate=0.5, mask_stride=None, spatial_repeat=True, temporal_shuffle=True,
    ...
    model = WCLIP(
            ...
            mask_rate=mask_rate, mask_stride=mask_stride,
            spatial_repeat=spatial_repeat, temporal_shuffle=temporal_shuffle,
            channel_fold=channel_fold, temporal_scale=temporal_scale,
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
        jit:bool = False, download_root: str = None, T=8, temporal_modeling_type=False,
         mask_rate=0.5, mask_stride=None, spatial_repeat=True, temporal_shuffle=True,
         channel_fold=64, temporal_scale=1,
         use_checkpoint=False, context_length = 77, num_experts=0, expert_insert_layers=[], record_routing=False, routing_type='patch-level'):
...
if __name__ == '__main__':
    model, preprocess = clip.load("/.cache/clip/ViT-B-16.pt", jit=False, )
