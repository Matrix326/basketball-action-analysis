...
def _quadratic_roots_z(
    p0z: float, v0z: float, t0: float, target: float, fps: float
) -> list[float]:
...
def _velocity_at(
    frames: np.ndarray, p0: np.ndarray, v0: np.ndarray, t0: float, fps: float
) -> np.ndarray:
...
def _try_bounce_bridge(
    ...
    try:
        ...
        p0_r, v0_r, _ = fit_ballistic(
            right_frames, right_pos, float(right_frames[0]), fps
        )
    except ValueError:
        return None
...
def _try_double_sided(
    ...
    try:
        p0, v0, rms = fit_ballistic(
            flank_frames, flank_pos, float(flank_frames[0]), fps
        )
    except ValueError:
        return None
...
def _fill_one_gap(
    ...
    for seg in segments:
        if seg.start <= gap_start and gap_end <= seg.end:
            ...
            if not any(gap_start <= bf <= gap_end for bf in bounce_frames):
                gap_frames = np.arange(gap_start, gap_end + 1, dtype=np.float64)
                sim = simulate_ballistic(
                    gap_frames, seg.p0, seg.v0, float(seg.start), fps
                )
    ...
    _left_sel = obs_frames <= grid[gap_start_idx - 1] if gap_start_idx > 0 else None
    # Observed frames before the gap (bounded window).
    before = (
        obs_frames[obs_frames <= grid[gap_start_idx - 1]]
        if gap_start_idx > 0
        else obs_frames[:0]
    )
    before = before[-flank_frames_back:]
    after = (
        obs_frames[obs_frames >= grid[gap_end_idx + 1]]
        if gap_end_idx + 1 < len(grid)
        else obs_frames[len(obs_frames) :]
    )
    ...
    for seg in segments:
        ...
        if seg.start <= last_obs <= seg.end:
            ...
            if tail_start <= grid[-1]:
                tail = np.arange(
                    tail_start,
                    min(int(grid[-1]), tail_start + max_extrapolate_frames) + 1,
                    dtype=np.float64,
                )
        ...
        if seg.start <= first_obs <= seg.end:
            ...
            if head_end >= grid[0]:
                head = np.arange(
                    max(int(grid[0]), head_end - max_extrapolate_frames + 1),
                    head_end + 1,
                    dtype=np.float64,
                )
