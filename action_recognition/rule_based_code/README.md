# 动作识别模块（action module）

多视角篮球流水线的**下游动作识别模块**：读取感知层的 `poses_3d.json`，
输出动作事件 `actions.json`（pass / shoot / layup / rebound / follow_up /
block + 命中判定 + 逐帧持球人）。

- **上游**：[`perception/`](../../perception/) —— 检测（含 hybrid 球检测）、
  RTMPose 姿态、ReID、3D 重建、轨迹与可视化。本模块不重复这些功能。
- **本模块**：球轨迹后处理（弹道分段/弹跳/状态）+ 规则引擎。**依赖只有
  numpy/scipy/yaml**，可在任意装了 numpy/scipy 的机器上运行。
- **不破坏上游接口**：本模块只读 `poses_3d[frame][id]`（17×3 关键点）、
  `quality`、`balls_2d`/`balls_3d` 字段；感知层的可视化路径完全不受影响。

**本 README 记录球轨迹后处理与规则引擎的完整逻辑、全部参数与实现要点**
（务必保留此文档，供后续维护与调参参考）。
...
│ RTMPose 姿态                  │ poses_   │    - 三角化篮筐（坐标系对齐）     │
│ ReID + 3D 重建                │ 3d.json  │ 2. 球轨迹后处理 src/             │
│ 可视化（俯视/骨架/轨迹平滑）    │ ───────▶ │    ball_trajectory/              │
└──────────────────────────────┘          │ 3. 规则引擎 src/action_rules/    │
                                          └───────────────┬────────────────┘
                                                          ▼
                                                   actions.json
                                        （事件 + 命中 + 逐帧 possession）
...
接入细节、坐标约定、对比实验结论见 [docs/integration.md](docs/integration.md)。
...
### 4.1 标准用法（接在 perception 后面）

```bash
# 感知层（perception/）：检测 + 姿态 + ReID + 3D + 可视化
cd ../../perception && python src/run_rfdetr_full_pipeline.py \
    --config config/config.yaml --start-frame 0 --end-frame 19739 \
    --output-base output/full

# 动作模块（本目录）：一条命令跑完整条动作链
python tools/run_action_pipeline.py \
    ...
    --intrinsics ../../perception/assets/intrinsics_parameters/undistorted_intrinsics_correct.json \
    --print-stats
# -> output/actions/actions.json（事件 + 命中 + 逐帧 possession）
```

分步调用（调试用）：

```bash
python tools/adapt_perception.py --poses <poses_3d.json> --out-dir <out> \
    --extrinsics <...>/extrinsics_new_calibration.json \
    --intrinsics <...>/undistorted_intrinsics_correct.json
python src/ball_trajectory/run_ball_trajectory.py --config config/config.yaml \
    --poses-json <out>/poses_3d.json --output <out>/ball_trajectory.json --print-stats
python src/action_rules/run_action_rules.py --config config/config.yaml \
    --ball-trajectory <out>/ball_trajectory.json --poses-json <out>/poses_3d.json \
    --hoop-3d <out>/hoop_3d.json --output <out>/actions.json
```
...
python src/action_rules/evaluate_gt.py \
    --gt <gt.json> --actions <actions.json> --id-map <id_map.json> \
    --start-frame 0 --end-frame 19739 --tolerance 50 --output /tmp/gt_eval.json
```

**评估口径**：类型匹配（GT_TO_OURS）+ 时间容差 50 帧 + actor（id_map 映射）。
**id_map 必须按 pipeline 重建**（每个 pipeline 的 track ID 是内部编号，互不通用；
用事件共现自动生成，见 docs/integration.md）。非可比类别（Defence 等）不计入。
...
| 参数 | 位置 | 说明 |
|---|---|---|
| `camera.extrinsics_path`（perception/config） | perception | 新外参 json（4 视角） |
| `camera.court_world_bounds`（perception/config） | perception | 场地世界系范围 `[xmin, xmax, ymin, ymax]` |
| `action_rules.three_point_radius_m` | **本模块 config** | **三分弧实测半径**（当前 5.5；换场地需重新拟合：8 点弧线拟合） |
| 篮筐 3D 位置 | 桥接器 | 由 `tools/adapt_perception.py` 用标注的篮筐像素 + 感知层标定**三角化**得到（也可 `--hoop-center x y z` 直接指定） |
| `ball.exclusion_zones`（perception/config） | perception | 各视角固定背景误检区（画面中重复被误检为球的静态物体）——换场地需重新标注 |
| `camera.intrinsics_path` | config | 相机换了才需要（内参畸变） |
| `ball.max_distance / max_jump` | config | 球检测距离上限/单帧位移上限（不同场地尺寸/相机高度可能要调） |

### 5.3 验证流程（换场地后必跑）

1. 篮筐 3D 位置：`perception/src/hoop_detection/run_hoop_detection.py`
   （YOLO 检测 + 标定三角化），或由本模块桥接器从标注像素三角化
...
## 8. 模型与数据（上游 perception 模块）

本模块不包含模型，也不直接处理视频：

| 资源 | 位置 |
|---|---|
| 人检测 | `perception/models/rfdetr-seg-2xlarge.b4.*`（2XL，真 mask） |
| 球检测（hybrid） | `perception/models/finetuned/rfdetr_large_ball_player.pth`（微调 Large） |
| 姿态 | `perception/models/rtmpose/`（RTMPose-m） |
| ReID | `perception/models/reid/` + `perception/models/insightface/` |
| 相机标定 | `perception/assets/{intrinsics,extrinsic}_parameters/` |
| 输入视频 | `perception/config/config.yaml` 的 `data_root` / `videos` |

球检测配置对比（900-1800 帧，同一动作引擎评估）：

| 配置 | 球 3D 观测覆盖 | 各视角球检出 |
|---|---|---|
| 2XL 单模型（通用） | 82.7% | 60-72% |
| **2XL 人 + 微调球（hybrid，当前默认）** | **88.7%** | **65-81%** |
