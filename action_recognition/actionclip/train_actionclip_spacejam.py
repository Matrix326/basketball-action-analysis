...
from torch.cuda.amp import (
    GradScaler,
    autocast,  # ty: ignore[deprecated]  # Keep legacy Torch support.
)
...
class SpaceJamDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # A transform is required for this path; errors retain the existing fallback.
            assert self.transform is not None
            video_tensor = self.transform(video_anno)
            return {"video": video_tensor, "label": label, "video_path": video_path}
...
def load_pretrained_text_features(
    model,
    label_map_path,
    device,
    template="The basketball player is {}",
    use_detailed_descriptions=False,
    action_descriptions_path=None,
):
    """加载预训练的文本特征"""
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast(enabled=scaler is not None):  # ty: ignore[deprecated]  # Keep legacy Torch support.
...
@torch.no_grad()
def validate(
    model, dataloader, text_features, device, criterion=None, is_distributed=False
):
    """验证函数"""
    model.eval()
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="ViT-B/16-16", help="ActionCLIP模型"
    )
    parser.add_argument("--epochs", type=int, default=5, help="训练轮数")
    parser.add_argument("--batch-size", type=int, default=32, help="批次大小")
    parser.add_argument("--lr", type=float, default=5e-6, help="学习率（adapter）")
    parser.add_argument("--weight-decay", type=float, default=0.01, help="权重衰减")
    parser.add_argument("--device", type=str, default="cuda", help="训练设备")
    parser.add_argument("--workers", type=int, default=8, help="数据加载线程数")
    parser.add_argument(
        "--use-distributed", action="store_true", help="使用 torchrun 启动的多卡训练"
    )
    parser.add_argument("--clip-len", type=int, default=16, help="每个视频采样帧数")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    try:
        ...
        scaler = GradScaler()  # ty: ignore[deprecated]  # Keep legacy Torch support.

        class_counts = [4381, 4715, 4744, 1431, 1876, 969, 3074, 3129, 5167, 9369]
        total = sum(class_counts)
        class_weights = [
            np.sqrt(total / (len(class_counts) * count)) for count in class_counts
        ]
        print("\n类别权重:")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if rank == 0:
                ...
                torch.save(
                    epoch_checkpoint, os.path.join(work_dir, f"epoch_{epoch}.pth")
                )
