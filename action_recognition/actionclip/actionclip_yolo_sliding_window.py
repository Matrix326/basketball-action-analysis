...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ActionCLIP Multi-Person Sliding Window Recognition"
    )
    parser.add_argument(
        "--video",
        type=str,
    ...
    parser.add_argument(
        "--start-frame", type=int, default=1200, help="起始帧索引（包含，0-based）"
    )
    parser.add_argument(
        "--end-frame",
        type=int,
        default=1800,
        help="结束帧索引（包含，-1表示到视频末尾）",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="/data/ljy23/project/stal/mm/mmaction2/projects/actionclip/work_dirs/actionclip_spacejam/20260810_102900/epoch_5.pth",
        help="ActionCLIP模型",
    )
    parser.add_argument(
        "--yolo-model",
    ...
    parser.add_argument(
        "--out-filename",
        type=str,
        default="/data/ljy23/project/stal/mm/mmaction2/projects/actionclip/work_dirs/actionclip_spacejam/20260810_102900/A1.mp4",
        help="输出视频文件名（为空则自动根据视频名和帧范围生成）",
    )
...
def load_video_frames(video_path, start_frame=0, end_frame=-1):
    ...
    cap.release()
    print(
        f"  视频总帧数: {total_frames}, 加载范围: [{start_frame}, {end_frame}], 实际加载: {len(frames)} 帧"
    )
    return frames
...
def track_persons(all_frame_detections, max_distance=100):
    ...
    """
    tracked_persons: list[PersonTrack] = []
        ...
        if frame_idx == 0:
            ...
            while unmatched_detections:
                ...
                if min_cost < max_distance:
                    # 匹配成功
                    tracked_persons[min_ti]["bboxes"][frame_idx] = detections[min_di][
                        "bbox"
                    ]
                    tracked_persons[min_ti]["centers"][frame_idx] = detections[min_di][
                        "center"
                    ]
...
def visualize_person_actions_sliding(
...
def main():
    ...
    print(
        f"帧范围: [{args.start_frame}, {args.end_frame if args.end_frame >= 0 else '末尾'}]"
    )
    ...
    print(
        f"  采样间隔: 每{sample_interval}帧抽1帧 → {args.window_len // sample_interval}帧给模型"
    )
    print(f"  模型输入帧数: {clip_len} 帧")
    print(
        f"  检测框处理模式: {'填充模式（左右补0）' if args.padding_mode else '裁剪模式（扩展成正方形）'}"
    )
    ...
    # 保存原始追踪数据（用于可视化）
    _original_tracked_persons = tracked_persons.copy()

    # 过滤掉检测框太少的球员
    tracked_persons = [
        tp for tp in tracked_persons if tp["num_valid_frames"] >= args.window_len
    ]
    ...
    # 创建保存视频片段的目录
    clips_output_dir = os.path.join(
        os.path.dirname(args.out_filename), "model_input_clips"
    )
    ...
    # 为每个球员提取帧并进行滑动窗口推理
    print("\n对每个球员进行滑动窗口推理...")
    persons_windows: list[list[dict]] = []

    for person_idx, person in enumerate(tracked_persons):
        person_window_results: list[dict] = []
                ...
                if fi < len(person["bboxes"]) and person["bboxes"][fi] is not None:
                    ...
                    if args.padding_mode:
                        ...
                        cropped = crop_and_resize_with_padding(
                            frames[fi], bbox, args.input_size, expand_ratio=1.2
                        )
                    else:
                        # 裁剪模式：扩展检测框成正方形
                        expanded_bbox = expand_bbox(
                            bbox, args.expand_ratio, frames[0].shape
                        )
                        processed_bbox = expanded_bbox
                        cropped = crop_and_resize(
                            frames[fi], expanded_bbox, args.input_size
                        )
        ...
        if person_window_results:
            ...
            for w in person_window_results:
                action_name = labels[cast(int, w["top1_label"])]
    ...
    if not output_path:
        ...
        frame_suffix = (
            f"_f{args.start_frame}_{args.end_frame if args.end_frame >= 0 else 'end'}"
        )
        ...
        output_path = os.path.join(
            "/data/ljy23/project/stal/mm/mmaction2/projects/actionclip", output_path
        )

    print("\n生成可视化视频...")
    visualize_person_actions_sliding(
        frames, persons_windows, tracked_persons, labels, output_path, args.fps
    )
