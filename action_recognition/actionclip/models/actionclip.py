...
def text_prompt(labels_or_label_file, templates_or_template_file=None):
    if isinstance(labels_or_label_file, str):
        ...
        raise ValueError(
            f"`labels_or_label_file` must be `list` or `str`, "
    ...
    if templates_or_template_file is None:
        ...
        raise ValueError(
            f"`template` must be list of `str`, `str` or `None`, "
            f"but got {type(templates_or_template_file)}"
        )

    num_prompt = len(templates)
    prompt = torch.cat([clip.tokenize(t.format(c)) for t in templates for c in labels])
...
class ActionClip(BaseModel):
    def __init__(
        ...
        self.adapter = TransformerAdapter(
            self.clip, num_adapter_segs, num_adapter_layers
        )
    ...
    def forward(
        ...
        if mode == "tensor":
            ...
            data_samples = cast(List, data_samples)
            ...
            # (bsz, num_views*num_prompt, num_classes)
            similarity = (100.0 * video_features @ text_features.T).view(
                bsz, num_views * self.num_prompt, -1
            )
            ...
            selected_prompt = self.prompt.view(
                self.num_prompt, -1, self.prompt.shape[-1]
            )[text_id, real_labels].to(inputs.device)
