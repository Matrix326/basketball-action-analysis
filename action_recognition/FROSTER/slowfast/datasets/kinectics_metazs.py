...
class Kinetics_metazs(torch.utils.data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (int): the sample index to be loaded.
        Returns:
        ...
        if self.mode == "train":
            classes = self.meta_zs.get_unique_classes()
            N, K = 4, 1
            spt_classes = random.sample(classes, N)
            qry_classes = random.sample(list(set(classes) - set(spt_classes)), N)

            spt_set, spt_label, spt_index = [], [], []
            qry_set, qry_label, qry_index = [], [], []

            for sc, qc in zip(spt_classes, qry_classes):
                sc_total = self.meta_zs.get_num_videos_for_class(sc)
                qc_total = self.meta_zs.get_num_videos_for_class(qc)
                sc_idxs = random.sample(range(sc_total), K)
                qc_idxs = random.sample(range(qc_total), K)
                for idx in sc_idxs:
                    spt_vid, spt_id = self.meta_zs.get_rand_vid(sc, idx)
                    frames, label, index, time_idx, meta = self.get_sq(spt_id)
                    spt_set.append(frames)
                    spt_label.append(label)
                    spt_index.append(index)
                for idx in qc_idxs:
                    qry_vid, qry_id = self.meta_zs.get_rand_vid(qc, idx)
                    frames, label, index, time_idx, meta = self.get_sq(qry_id)
                    qry_set.append(frames)
                    qry_label.append(label)
                    qry_index.append(index)
            s = list(zip(spt_set, spt_label, spt_index))
            random.shuffle(s)
            spt_set, spt_label, spt_index = zip(*s)
            spt_set = torch.stack(spt_set)

            q = list(zip(qry_set, qry_label, qry_index))
            random.shuffle(q)
