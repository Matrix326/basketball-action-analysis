...
import copy
from typing import Optional, Tuple as Tuple, Union as Union, cast
...
class TimesAttentionBlock(nn.Module):
    def __init__(
        ...
        self.attn = activation.MultiheadAttention(
            d_model, n_head, temporal_shift=temporal_modeling_type, T=T
        )
...
class ChannelShiftAttentionBlock(nn.Module):
    def __init__(
        ...
        # type: channel_shift or expand_temporal_view
        self.attn = activation.MultiheadAttention(
            d_model, n_head, temporal_shift="channel_shift"
        )
        self.ln_1 = LayerNorm(d_model)
...
class CrossFramelAttentionBlock(nn.Module):
    def __init__(
        ...
        if num_experts > 0:
            self.experts_head = nn.Sequential(
                *[
                    nn.Sequential(
                        OrderedDict(
                            [
            ...
            self.experts_tail = nn.Sequential(
                *[
                    nn.Sequential(
                        OrderedDict([("c_proj", nn.Linear(d_model * 4, d_model))])
                    )
                    for _ in range(num_experts)
                ]
            )
    ...
    def forward(self, x):
        ...
        sequence_length, bt, d = x.size()
        b = bt // self.T
        x = x.view(sequence_length, b, self.T, d)
        ...
        x = x.view(sequence_length + 1, -1, d)
        x = x + self.attention(self.ln_1(x))
        x = x[:sequence_length, :, :]
        ...
        if self.num_experts > 0:
            ...
            [
                output_head.append(cast(nn.Sequential, self.experts_head[i])[0](ln_x))
            ...
            [
                output.append(self.experts_tail[i](output_head))
                for i in range(self.num_experts)
            ]
            rout2 = torch.nn.functional.softmax(
                self.routing2(output_head), -1
            ).unsqueeze(-1)
...
class STAdaptAttentionBlock(nn.Module):
    def __init__(
        ...
        self.stadapt_conv3d_1 = nn.Conv3d(
            d_model // 2,
            d_model // 2,
            kernel_size=(3, 1, 1),
            padding="same",
...
class STAdaptZeroInitAttentionBlock(nn.Module):
    def __init__(
        ...
        nn.init.constant_(cast(torch.Tensor, self.stadapt_up_1.bias), 0)
...
class STCrossAttentionBlock(nn.Module):
    def __init__(
        ...
        self.mask_stride = [1, 2, 2] if mask_stride is None else mask_stride
    ...
    def _window_masking(self, x: torch.Tensor):
        ...
        x = rearrange(
            x,
            "b (t s0) (h s1) (w s2) d -> b t (h w) (s0 s1 s2) d",
        ...
        test_mask = rearrange(
            test_mask,
            "(t s0) (h s1) (w s2) -> t (h w) (s0 s1 s2)",
            s0=self.mask_stride[0],
            s1=self.mask_stride[1],
            s2=self.mask_stride[2],
        )  # 8 (7 7) (2 2)
        mask_patch_per_cell = mask_num // (T * spatial_cell_num)
        mask_list = [1 for i in range(mask_patch_per_cell)] + [
            0 for i in range(test_mask.size(2) - mask_patch_per_cell)
        ]

        for t in range(T):
            offset = t % test_mask.size(-1)
            test_mask[t, :, :] = torch.Tensor(
                mask_list[-offset:] + mask_list[:-offset]
            )[None, :]
        ...
        for i in range(self.mask_stride[1] * self.mask_stride[2]):
            train_mask = torch.zeros(
                T, self.mask_stride[1] * self.mask_stride[2], device=x.device
            )  # 8 (2 2)
            for t in range(T):
                offset = (t + i) % train_mask.size(-1)
                train_mask[t, :] = torch.Tensor(
                    mask_list[-offset:] + mask_list[:-offset]
                )
        ...
        if self.training:
            if self.spatial_repeat:
                ...
                mask_index = torch.randint(
                    K, (B, spatial_cell_num), device=x.device
                ).flatten()
            selected_mask = train_mask.to(x.device)[mask_index, ...].view(
                B, spatial_cell_num, T, -1
            )  # B 49 T 4
            ...
            if self.temporal_shuffle:
                ...
                selected_mask = torch.gather(
                    selected_mask,
                    index=temporal_index[:, :, None, None].expand_as(selected_mask),
                    dim=1,
                )
        else:
            selected_mask = (
                test_mask.flatten(-2)[None, ...].to(x.device).repeat(B, 1, 1)
            )  # B T (49*4)
            ...
            if self.temporal_shuffle:
                ...
                selected_mask = torch.gather(
                    selected_mask,
                    index=temporal_index[:, :, None].expand_as(selected_mask),
                    dim=1,
                )
        ...
        x_mask = (
            x[~selected_mask].reshape(B, T, -1, D).permute(2, 0, 1, 3)
        )  # B T 196//2 D -> 196//2 B T D
    ...
    def _channel_mixing(self, x: torch.Tensor, scale: int = 1):
        ...
        x = x.reshape(sequence_length, -1, self.T, d)  # l b t d
        # x_cls = x[0,:,:,:].unsqueeze(dim=0)
        x = x[1:, ...]
        ...
        for i in range(scale):
            x_mix[:, :, (i + 1) :, :d] = x[:, :, : -(i + 1), :d]
            x_mix[:, :, : -(i + 1), (2 * init_dim - d) : 2 * init_dim] = x[
                :, :, (i + 1) :, (2 * init_dim - d) : 2 * init_dim
            ]
            d //= 2
    ...
    def _padding(self, x_pre: torch.Tensor, x_post: torch.Tensor, mask: torch.Tensor):
        ...
        out_x = rearrange(
            out_x,
            "b t (h w) (s0 s1 s2) d -> b (t s0) (h s1) (w s2) d",
            s0=self.mask_stride[0],
            s1=self.mask_stride[1],
            s2=self.mask_stride[2],
            h=H,
    ...
    def forward(self, x):
        ...
        if self.num_experts > 0:
            ...
            if self.routing_type == "patch-level":
                rout1 = torch.nn.functional.softmax(self.routing1(ln_x), -1).unsqueeze(
                    -1
                )
            elif self.routing_type == "image-level":
                rout1 = torch.nn.functional.softmax(
                    self.routing1(ln_x[0].unsqueeze(0)), -1
                ).unsqueeze(-1)
            ...
            if self.routing_type == "patch-level":
                ...
                rout2 = torch.nn.functional.softmax(
                    self.routing2(output_head[0].unsqueeze(0)), -1
                ).unsqueeze(-1)
            output = torch.stack(output, 0).permute(1, 2, 0, 3)
            output = (rout2 * output).sum(-2)
...
class TSTransformer(nn.Module):
    def __init__(
        ...
        if self.temporal_modeling_type is None:
            self.resblocks = nn.Sequential(
                *[
                    ...
                    if layer_id in expert_insert_layers
                    else ResidualAttentionBlock(
                        width,
                        heads,
                        attn_mask,
            ...
            or self.temporal_modeling_type == "expand_temporal_view_step2"
            or self.temporal_modeling_type == "expand_temporal_view_step3"
        ):
            self.resblocks = nn.Sequential(
                *[
                    TimesAttentionBlock(
                        ...
                        attn_mask,
                        T=T,
                        temporal_modeling_type=self.temporal_modeling_type,
                    )
            ...
            self.resblocks = nn.Sequential(
                *[
                    ChannelShiftAttentionBlock(width, heads, attn_mask, T=T)
                    for _ in range(layers)
                ]
            )
            ...
            self.resblocks = nn.Sequential(
                *[
                    ...
                    if layer_id in expert_insert_layers
                    else CrossFramelAttentionBlock(
                        width, heads, attn_mask, T=T, record_routing=record_routing
                    )
                    for layer_id in range(layers)
                ]
            )
            ...
            self.resblocks = nn.Sequential(
                *[
                    ...
                    if layer_id in expert_insert_layers
                    else STAdaptZeroInitAttentionBlock(
                        width, heads, attn_mask, T=T, record_routing=record_routing
                    )
                    for layer_id in range(layers)
                ]
...
if __name__ == "__main__":
    ...
    # fast update
    with torch.cuda.amp.autocast(enabled=True):  # ty: ignore[deprecated]  # Keep Torch 1.12 compatibility.
        y = model(x)
    ...
    grads_record = {
        name: params.grad.clone().detach() if params.grad is not None else params.grad
