...
from . import clip, customize_visiontransformer as customize_visiontransformer
from .build import MODEL_REGISTRY
from .clip.clip import _MODELS, _download, available_models
from .clip.model import CLIP, LayerNorm, Transformer, convert_weights
...
class TemporalClipVideo(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.tune_head:
            if self.text_aug:
                self.dynamic_classifier = self.achieve_xcsf_matrix(
                    self.text_feature_file
                )
            else:
                self.dynamic_classifier = self.achieve_csf_matrix(
                    self.text_dict, self.model
                )
            ...
            self.prompt_embed = torch.nn.Parameter(
                torch.rand(int(self.prompt_num), embedding_dim).cuda(),
                requires_grad=True,
            )
            torch.nn.init.normal_(self.prompt_embed, std=0.01)

            id2cls = {}
            for idx, cls in json.load(
                open(cfg.DATA.INDEX_LABEL_MAPPING_FILE, "r")
            ).items():
                id2cls[int(idx)] = cls
            self.classnames = [id2cls[i] for i in range(len(id2cls))]
            prompts = [
                " ".join(["X"] * self.prompt_num) + " " + name + "."
                for name in self.classnames
            ]
            tokenized_prompts = torch.cat(
                [clip.tokenize(p, context_length=self.context_length) for p in prompts]
            )
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.MODEL.ARCH == "vitb32":
            self.model, self.preprocess = load(
                ...
                mask_rate=cfg.MODEL.MASK_RATE,
                mask_stride=cfg.MODEL.MASK_STRIDE,
                spatial_repeat=cfg.MODEL.SPATIAL_REPEAT,
                temporal_shuffle=cfg.MODEL.TEMPORAL_SHUFFLE,
                channel_fold=cfg.MODEL.CHANNEL_FOLD,
                temporal_scale=cfg.MODEL.TEMPORAL_SCALE,
        ...
        if self.training:
            ...
            if self.tune_head:
                ...
                text_embedding = torch.cat(
                    (
                        self.token_prefix,
                        self.prompt_embed.unsqueeze(0).expand(
                            len(self.classnames), -1, -1
                        ),
                        self.token_suffix,
                    ),
                    1,
                )
                norm_head = self.model.prompt_encode_text(
                    text_embedding,
                ...
                if len(self.dynamic_classifier.shape) < 3:
                    ...
                    sims = torch.einsum(
                        "btd,knd->btkn",
                        self.model.logit_scale.exp() * img_encode,
                    ...
                    if self.cfg.TRAIN.CROSS_BATCH_META_DEBIAS:
                        ...
                        pred = (max_mean_n + max_mean_t) / 2.0  # b k

            if self.cfg.TRAIN.CROSS_BATCH_META_DEBIAS:
                pred = pred.reshape(bz, clip_len, -1)
            else:
                pred = (
                    pred.reshape(bz, clip_len, -1).mean(1)
                    if pred.shape[0] != bz
                    else pred
                )
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation):
                with torch.no_grad():
                    ...
                    raw_img_encode = raw_img_encode / raw_img_encode.norm(
                        dim=-1, keepdim=True
                    )
                    ...
                    if len(self.dynamic_classifier.shape) < 3:
                        ...
                        sims = torch.einsum(
                            "btd,knd->btkn",
                            self.model.logit_scale.exp() * raw_img_encode,
                            self.dynamic_classifier,
                        )
                        max_t, _ = torch.max(sims, dim=1, keepdim=False)  # b k n
                        max_mean_n = torch.mean(max_t, dim=-1, keepdim=False)  # b k
                        max_n, _ = torch.max(sims, dim=-1, keepdim=False)  # b t k
                        max_mean_t = torch.mean(max_n, dim=1, keepdim=False)  # b k
                        raw_pred = (max_mean_n + max_mean_t) / 2.0  # b k
            ...
            pred = (
                pred.reshape(bz, clip_len, -1).mean(1) if pred.shape[0] != bz else pred
            )
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation):
                with torch.no_grad():
                    ...
                    raw_pred = (
                        raw_pred.reshape(bz, clip_len, -1).mean(1)
                        if raw_pred.shape[0] != bz
                        else raw_pred
                    )
    ...
    def achieve_xcsf_matrix(self, text_feature_file):
        if os.path.isfile(text_feature_file):
            ...
            xcsf_matrix = text_feature / text_feature.norm(
                dim=-1, keepdim=True
            )  # k a d
            return xcsf_matrix
        else:
...
class WCLIP(CLIP):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            torch.empty(max(self.context_length, 77), transformer_width)
        )
    ...
    def prompt_encode_text(
        ...
        x = (
            prompts
            + self.positional_embedding.type(self.dtype)[: self.context_length, :]
        )
        x = x.permute(1, 0, 2)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)
