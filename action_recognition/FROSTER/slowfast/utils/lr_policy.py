...
def lr_func_cosine(cfg, cur_epoch):
    ...
    if cfg.SOLVER.COSINE_RESTART_EPOCH == 0.0:
        return (
        ...
        restart_epoch = (cur_epoch - offset) % cfg.SOLVER.COSINE_RESTART_EPOCH
        return (
            cfg.SOLVER.COSINE_END_LR
            + (cfg.SOLVER.BASE_LR - cfg.SOLVER.COSINE_END_LR)
            * (
                math.cos(math.pi * restart_epoch / cfg.SOLVER.COSINE_RESTART_EPOCH)
                + 1.0
            )
            * 0.5
        )
