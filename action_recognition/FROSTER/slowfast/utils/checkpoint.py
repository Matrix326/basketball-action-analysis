...
def get_last_checkpoint(path_to_job, task):
    ...
    if len(names) == 0:
        return None
    # Sort the checkpoints by epoch and pick the latest one that is
    # non-empty. A zero-byte file means a previous save was interrupted
    # (e.g. the process was killed while writing); loading it would crash
    # with EOFError. In-progress atomic saves are skipped via the .tmp
    # suffix (see save_checkpoint).
    for name in sorted(names, reverse=True):
        ...
        try:
            ...
            # File vanished mid-scan (concurrent writer); skip it.
            continue
    return None
...
def has_checkpoint(path_to_job):
    ...
    for f in files:
        if "checkpoint" not in f or f.endswith(".tmp"):
            continue
        try:
...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, scaler=None):
    ...
    # Write the checkpoint atomically: save to a temp file first, then
    # rename into place. An interrupted save (e.g. killed mid-write or
    # disk full) then never leaves a truncated file that would break the
    # next resume -- the previous valid checkpoint stays intact.
    path_to_checkpoint = get_path_to_checkpoint(path_to_job, epoch + 1, cfg.TASK)
    tmp_path_to_checkpoint = path_to_checkpoint + ".tmp"
    with cast(BinaryIO, pathmgr.open(tmp_path_to_checkpoint, "wb")) as f:
        torch.save(checkpoint, f)
    os.replace(tmp_path_to_checkpoint, path_to_checkpoint)
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        with cast(BinaryIO, pathmgr.open(path_to_checkpoint, "rb")) as f:
