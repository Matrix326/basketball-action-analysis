...
def average_checkpoint(checkpoint_list):
    ...
    for key in raw_clip_weight:
        ...
        for ckpt in new_ckpt_list:
            cast(list[torch.Tensor], state_dict[key]).append(ckpt[1][key])
    ...
    for key in state_dict:
        try:
            state_dict[key] = torch.mean(
                torch.stack(cast(list[torch.Tensor], state_dict[key]), 0), 0
            )
...
checkpoint_list = [
    (os.path.join(source_dir, i), int(i.split(".")[0].split("_")[-1]))
...
torch.save(
    {"model_state": swa_state_dict},
    os.path.join(output_dir, "swa_%d_%d.pth" % (wa_start, wa_end)),
)
