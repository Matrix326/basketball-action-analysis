...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/data/ljy23/project/vlm/FROSTER/output/teacher/checkpoints/checkpoint_epoch_00018.pyth",
        help="FROSTER模型权重路径 (.pyth 文件)",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--end-frame", type=int, default=1800, help="结束帧索引(-1到末尾)"
    )
    parser.add_argument(
        "--yolo-model",
        type=str,
...
def main():
    ...
    if args.skeleton_mode:
        ...
        for fi, frame in enumerate(frames):
            results = pose_model(
                frame, verbose=False, conf=args.skeleton_pose_conf, classes=[0]
            )
