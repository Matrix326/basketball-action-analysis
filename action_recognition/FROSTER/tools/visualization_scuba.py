...
def run_visualization(vis_loader, model, cfg, writer=None):
    ...
    for inputs, labels, _ in tqdm.tqdm(vis_loader):
        ...
        if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.ENABLE:
            ...
            curr_inp = data_utils.revert_tensor_normalize(curr_inp,cfg.DATA.MEAN,cfg.DATA.STD)
                    ...
                    if (
                        ...
                        for path_idx, input_pathway in enumerate(cur_input):
                            if cfg.TEST.DATASET == "ava" and cfg.AVA.BGR:
                                ...
                                raw_video = raw_cur_input[path_idx][cur_batch_idx].permute(1,2,3,0)
                                raw_video = data_utils.revert_tensor_normalize(raw_video, cfg.DATA.MEAN, cfg.DATA.STD)
...
def reshape_transform(tensor, height=14, width=14):
    ...
    result = tensor[:,1:,:].reshape(tensor.size(0), height, width, tensor.size(-1)) # N L D -> N H W D
