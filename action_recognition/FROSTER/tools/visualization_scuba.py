...
def run_visualization(vis_loader, model, cfg, writer=None):
    ...
    if n_devices > 1:
        ...
        _grad_cam_layer_ls = cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.LAYER_LIST

    if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.ENABLE:
        # Older GradCAM releases select CUDA through a constructor keyword.
        cam_kwargs = {}
        if "use_cuda" in inspect.signature(GradCAM).parameters:
            cam_kwargs["use_cuda"] = torch.cuda.is_available()
        gradcam = GradCAM(
            model=model,
            target_layers=[model.module.model.visual.transformer.resblocks[-6].ln_2],
            **cam_kwargs,
    ...
    global_idx = -1
    for inputs, labels, _ in tqdm.tqdm(vis_loader):
        inputs = [inputs]
        if cfg.NUM_GPUS:
        ...
        raw_inputs = [inputs[0].clone().detach()]
        if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.ENABLE:
            # if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.USE_TRUE_LABEL:
            grayscale_cam = gradcam(input_tensor=inputs[0], targets=None)
            B, C, T, H, W = inputs[0].shape
            curr_inp = inputs[0].permute(0, 2, 3, 4, 1).cpu()  # b c t h w -> b t h w c
            curr_inp = data_utils.revert_tensor_normalize(
                curr_inp, cfg.DATA.MEAN, cfg.DATA.STD
            )
            curr_inp = curr_inp.flatten(0, 1)  # b t h w c -> b*t h w c
            # logger.info('1111111')
            # logger.info(curr_inp.shape)
            # logger.info('2222222')
            # logger.info(grayscale_cam.shape)
            inputs = []
            # torch.save(curr_inp, 'curr_inp.pth')
            for i in range(curr_inp.shape[0]):
                cur = show_cam_on_image(curr_inp[i].numpy(), grayscale_cam[i])
                cv2.cvtColor(cur, cv2.COLOR_RGB2BGR, cur)
                inputs.append(torch.from_numpy(cur).to("cuda"))
            inputs = [torch.cat(inputs, 0).reshape(B, T, H, W, C)]

            # inputs = show_cam_on_image(curr_inp, grayscale_cam)
            # inputs = [torch.from_numpy(inputs).reshape(B,T,H,W,C).to('cuda')] # b t h w c
        # if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.ENABLE:
        #     if cfg.TENSORBOARD.MODEL_VIS.GRAD_CAM.USE_TRUE_LABEL:
        #         inputs, preds = gradcam(input_tensor=inputs, targets=labels)
        #     else:
        #         inputs, preds = gradcam(inputs)
        if cfg.NUM_GPUS:
            inputs = du.all_gather_unaligned(inputs)
            raw_inputs = du.all_gather_unaligned(raw_inputs)
            ...
            if isinstance(inputs[0], list):
                for i in range(len(inputs)):
                    for j in range(len(inputs[0])):
                        ...
                        raw_inputs[i][j] = raw_inputs[i][j].cpu()
            else:
                inputs = [inp.cpu() for inp in inputs]
                raw_inputs = [inp.cpu() for inp in raw_inputs]
                    ...
                    if (
                        ...
                        for path_idx, input_pathway in enumerate(cur_input):
                            if cfg.TEST.DATASET == "ava" and cfg.AVA.BGR:
                                ...
                                raw_video = raw_cur_input[path_idx][
                                    cur_batch_idx
                                ].permute(1, 2, 3, 0)
                                raw_video = data_utils.revert_tensor_normalize(
                                    raw_video, cfg.DATA.MEAN, cfg.DATA.STD
                                )
                            ...
                            # logger.info('111111')
                            # logger.info(video.shape) # 3,8,224,224
                            # video = video.permute(1,2,3,0)
                            ...
                            raw_video = video_vis.draw_clip(
                                raw_video, cur_prediction, bboxes=bboxes
                            )
                            raw_video = (
                                torch.from_numpy(np.array(raw_video))
                                .permute(0, 3, 1, 2)
                                .unsqueeze(0)
                            )
                            writer.add_video(
...
def reshape_transform(tensor, height=14, width=14):
    tensor = tensor.transpose(0, 1)  # L N D -> N L D
    result = tensor[:, 1:, :].reshape(
        tensor.size(0), height, width, tensor.size(-1)
    )  # N L D -> N H W D

    result = result.permute(0, 3, 1, 2)  # N H W D -> N D H W
    return result
...
#         target_layer = [model.module.model.transformer.resblocks[11].attn]
#         # use_cuda = torch.cuda.is_available()
#         cam = PyGradCAM(model=model, target_layers=target_layer, use_cuda=True, reshape_transform=reshape_transform)
#         target_category = None
#         input_tensor = torch.randn([2, 3, 8, 224, 224])
...
def visualize(cfg):
    ...
    if cfg.TENSORBOARD.ENABLE and (
        ...
        vis_loader = loader.construct_frame_loader(cfg, "test")
