...
from mpl_toolkits.mplot3d.axis3d import XAxis, YAxis
import numpy as np
...
def create_multi_view_animation(
    ...
    balls_3d: Optional[Dict] = None,
    balls_3d_predicted: Optional[Dict] = None,
    ...
    for frame_idx, frame_num in enumerate(frames_to_process):
        ...
        for v_idx, (elev, azim, title) in enumerate(views):
            ...
            cast(XAxis, ax.xaxis).pane.fill = False
            cast(YAxis, ax.yaxis).pane.fill = False
            ax.zaxis.pane.fill = False

            cast(XAxis, ax.xaxis).pane.set_edgecolor("gray")
            cast(YAxis, ax.yaxis).pane.set_edgecolor("gray")
        ...
        fig.canvas.draw()
        img = np.frombuffer(
            cast(FigureCanvasAgg, fig.canvas).buffer_rgba(), dtype=np.uint8
        )
...
if __name__ == "__main__":
    ...
    create_multi_view_animation(
        ...
        + int(cfg.get("trajectory.process_seconds", 30) * source_fps)
        - 1,
        balls_3d=balls_3d,
        balls_3d_predicted=balls_3d_predicted,
        source_fps=source_fps,
        animation_fps=float(
            cfg.get("visualization.skeleton_3d.animation_fps", source_fps)
        ),
        ...
        ball_max_gap_frames=int(
            cfg.get("visualization.skeleton_3d.ball_max_gap_frames", 6)
        ),
        ball_trail_seconds=float(
            cfg.get("visualization.skeleton_3d.ball_trail_seconds", 2.0)
        ),
    )
