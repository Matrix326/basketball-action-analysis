...
def test_trajectory_video_applies_offset_without_changing_json_frame_ids(tmp_path):
    ...
    writer.release()
    data = {"poses_3d": {str(f): {"1": np.zeros((17, 3)).tolist()} for f in range(2, 8)},
    ...
    tracker = PlayerTrajectoryTracker3D(
        output_root_dir=str(tmp_path / "output"), input_video_path=str(source),
        poses_3d_json_path=str(poses), start_frame=2, process_seconds=6 / 30,
        fps=30, target_view="view1", app_config=config)
    tracker.process()
