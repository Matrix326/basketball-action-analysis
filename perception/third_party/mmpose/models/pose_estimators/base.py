...
class BasePoseEstimator(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        super().__init__(
            data_preprocessor=data_preprocessor, init_cfg=cast(Optional[dict], init_cfg)
        )
        ...
        neck, head = check_and_update_config(neck, cast(ConfigType, head))
    ...
    @staticmethod
    def _load_metainfo(metainfo: Optional[dict] = None) -> Optional[dict]:
    ...
    # MMEngine omits the documented tensor-mode return from its base annotation.
    def forward(  # ty: ignore[invalid-method-override]
        ...
        if mode == "loss":
            data_samples = cast(SampleList, data_samples)
    ...
    def _load_state_dict_pre_hook(
        self, state_dict, prefix, local_meta, *args, **kwargs
    ):
        """A hook function to.
