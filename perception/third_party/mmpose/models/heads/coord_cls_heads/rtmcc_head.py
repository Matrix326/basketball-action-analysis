...
from mmpose._data_types import PixelData
...
class RTMCCHead(BaseHead):
    ...
    def predict(
        ...
        test_cfg = cast(dict, test_cfg)
        if test_cfg.get("flip_test", False):
            # TTA: flip test -> feats = [orig, flipped]
            assert isinstance(feats, list) and len(feats) == 2
            flip_indices = cast(SampleList, batch_data_samples)[0].metainfo[
                "flip_indices"
            ]
            _feats, _feats_flip = feats

            _batch_pred_x, _batch_pred_y = self.forward(
                cast(Tuple[Tensor, ...], _feats)
            )

            _batch_pred_x_flip, _batch_pred_y_flip = self.forward(
                cast(Tuple[Tensor, ...], _feats_flip)
            )
            ...
            batch_pred_x, batch_pred_y = self.forward(cast(Tuple[Tensor, ...], feats))
        ...
        if test_cfg.get("output_heatmaps", False):
            ...
            for pred_instances, pred_x, pred_y in zip(
                preds,
                cast(np.ndarray, to_numpy(batch_pred_x)),
                cast(np.ndarray, to_numpy(batch_pred_y)),
            ):
    ...
    def loss(
        ...
        batch_data_samples = cast(SampleList, batch_data_samples)
        ...
        # calculate losses
        return dict(loss_kpt=self.loss_module(pred_simcc, gt_simcc, keypoint_weights))
