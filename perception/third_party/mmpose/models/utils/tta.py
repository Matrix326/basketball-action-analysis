...
def flip_vectors(x_labels: Tensor, y_labels: Tensor, flip_indices: List[int]):
    ...
    assert (
        len(flip_indices) == x_labels.shape[1]
        and len(flip_indices) == y_labels.shape[1]
    )
    ...
    for i, _heatmaps in enumerate(heatmaps):
        ...
        if _heatmaps.shape[2:4] != (h, w):
            heatmaps[i] = F.interpolate(
                _heatmaps, size=(h, w), mode="bilinear", align_corners=align_corners
            )

    if mode == "average":
        output = cast(Tensor, sum(heatmaps)).div(len(heatmaps))
    elif mode == "concat":
