...
import torch
from torch import Tensor, Tensor as _RegisteredTensor
...
class PatchMerging(BaseModule):
    ...
    def forward(self, x, input_size):
        ...
        out_h = (
            H
            + 2 * cast(tuple[int, int], self.sampler.padding)[0]
            - cast(tuple[int, int], self.sampler.dilation)[0]
            * (cast(tuple[int, int], self.sampler.kernel_size)[0] - 1)
            - 1
        ) // cast(tuple[int, int], self.sampler.stride)[0] + 1
        out_w = (
            W
            + 2 * cast(tuple[int, int], self.sampler.padding)[1]
            - cast(tuple[int, int], self.sampler.dilation)[1]
            * (cast(tuple[int, int], self.sampler.kernel_size)[1] - 1)
            - 1
        ) // cast(tuple[int, int], self.sampler.stride)[1] + 1
...
class ScaleNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.onnx.is_in_onnx_export() and digit_version(
            TORCH_VERSION
        ) >= digit_version("1.12"):
...
class SinePositionalEncoding(nn.Module):
    ...
    def generate_pos_encoding(
        ...
        position: Optional[Optional[Tensor]] = None,
    ):
        ...
        if (
            (not (self.learnable and self.training))
            and size is not None
            and hasattr(self, f"pos_enc_{size}")
        ):
        ...
        if self.spatial_dim == 1:
            if size is not None:
                ...
                position = torch.arange(
                    cast(int, size), dtype=torch.float32, device=self.dim_t.device
                )

            position = cast(Tensor, position)
    ...
    @staticmethod
    def apply_additional_pos_enc(
        feature: Tensor, pos_enc: Tensor, spatial_dim: int = 1
    ):
        ...
        assert spatial_dim in (1, 2), (
            f"the argument spatial_dim must be either 1 or 2, but got {spatial_dim}"
        )
        if spatial_dim == 2:
...
class DetrTransformerEncoderLayer(BaseModule):
    ...
    def __init__(
        self,
        self_attn_cfg: ConfigType = dict(embed_dims=256, num_heads=8, dropout=0.0),
