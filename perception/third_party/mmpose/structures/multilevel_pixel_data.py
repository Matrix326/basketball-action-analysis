...
class MultilevelPixelData(BaseDataElement):
    ...
    def __getitem__(
        self, item: Union[int, str, list, slice]
    ) -> Union[PixelData, Sequence]:
        ...
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support index type {type(item)}"
        )

    def levels(self) -> List[PixelData]:
        if self.nlevel:
            return [cast(PixelData, self[i]) for i in range(self.nlevel)]
        return []
    ...
    def set_field(
        ...
        if field_type == "metainfo":
            ...
            for i, v in enumerate(value):
                cast(PixelData, self[i]).set_field(v, name, field_type="data")
    ...
    def __getattr__(self, name):
        if (
            name in {"_data_fields", "_metainfo_fields"}
    ...
    def _convert(
        self, apply_to: Type, func: Callable[[Any], Any]
    ) -> "MultilevelPixelData":
        ...
        """
        new_data = cast(MultilevelPixelData, self.new())
